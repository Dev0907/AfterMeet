...
const TeamsList = () => {
    ...
    useEffect(() => {
        const fetchTeams = async () => {
            ...
            try {
                const { data } = await teamsApi.getAll(userId);
                setTeams(data);
            } catch (err) {
    ...
    const handleCreateTeam = async (e) => {
        ...
        try {
            const { data } = await teamsApi.create({ name: newTeamName.trim(), userId });
    ...
    const handleJoinTeam = async (e) => {
        ...
        try {
            const { data } = await teamsApi.join({ inviteCode: joinCode.trim().toUpperCase(), userId });
            const teamsResponse = await teamsApi.getAll(userId);
            setTeams(teamsResponse.data);
            setJoinCode('');
            setShowJoinModal(false);
            navigate(`/teams/${data.teamId}/meetings`);
        } catch (err) {
            setJoinError(err.response?.data?.error || 'Failed to join team. Please try again.');
        }
    };
    ...
    const getRoleBadge = (role) => {
        ...
        return (
            <span className={`text-xs font-bold uppercase px-2 py-0.5 border-2 border-black ${styles[role] || styles.member}`}>
                {role}
            </span>
        );
    };
        ...
        <div className="min-h-screen bg-neo-white flex flex-col">
            <header className="bg-neo-yellow border-b-4 border-black">
                <div className="max-w-6xl mx-auto px-4 py-6">
                    <div className="flex flex-wrap items-center justify-between gap-4">
                        <div>
                            <h1 className="text-4xl font-black uppercase tracking-tight">Your Teams</h1>
                            <p className="font-medium text-gray-700 mt-1">Select a team to view its meetings</p>
                        </div>
                        <div className="flex gap-2">
                            <NeoButton onClick={() => setShowJoinModal(true)} className="bg-white">
                                <Link2 size={18} className="mr-2" />
                                Join Team
                            </NeoButton>
                            <NeoButton onClick={() => setShowCreateModal(true)} className="bg-neo-teal">
            ...
            <main className="max-w-6xl mx-auto px-4 py-8 w-full flex-grow">
                {isLoading ? (
                    ...
                    <NeoCard className="text-center py-12">
                        ...
                        <p className="font-medium text-gray-600 mb-6">Create your first team to start organizing meetings</p>
                        <NeoButton onClick={() => setShowCreateModal(true)} className="bg-neo-teal">
                            <Plus size={18} className="mr-2" />
                            Create Team
                        </NeoButton>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {teams.map((team) => (
                            <button
                                ...
                                className="text-left bg-white border-4 border-black shadow-neo p-5 hover:translate-x-1 hover:translate-y-1 hover:shadow-neo-hover active:translate-x-2 active:translate-y-2 active:shadow-none transition-all group"
                            >
                                ...
                                <h3 className="font-black text-xl mb-3">{team.name}</h3>
            ...
            {showJoinModal && (
                <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
                    <NeoCard className="w-full max-w-md relative">
                        ...
                        <p className="text-sm font-medium text-gray-600 mb-4">Enter the invite code shared by the team host</p>
            ...
            <footer className="mt-12 border-t-4 border-black bg-neo-yellow py-6">
                <div className="max-w-6xl mx-auto px-4 text-center">
                    <p className="font-bold text-sm">AfterMeet • Meeting Intelligence Platform</p>
                </div>
            </footer>
        </div>
    );
